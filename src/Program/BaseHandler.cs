...
namespace Ucu.Poo.TelegramBot
{
    ...
    public abstract class BaseHandler : IHandler
    {
        ...
        public BaseHandler(IHandler next, List<Person> persons)
        {
            this.Next = next;
        ...
        protected abstract bool InternalHandle(Message message, out string response, out int responseCode, out Person person);
        ...
        protected virtual bool CanHandle(Message message, Person person, string role)
        {
            // Cuando no hay palabras clave este método debe ser sobreescrito por las clases sucesoras y por lo tanto
            // este método no debería haberse invocado.
            if (person.Role != role)
            {
                throw new InvalidOperationException("usted no tiene acceso a esta funcionalidad por su rol.");
            }
        ...
        public IHandler Handle(Message message, out string response, out int responseCode, out Person person)
        {
            if (this.InternalHandle(message, out response, out responseCode, out person))
            {
                return this;
            }
            else if (this.Next != null)
            {
                return this.Next.Handle(message, out response, out responseCode, out person);
            }
            else
            {
                return null;
